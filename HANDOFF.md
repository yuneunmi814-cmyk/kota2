...
|---|---|---|---:|
| E. 모바일 언어 메뉴 | ✅ 터치 메뉴·320px 잘림 수정, 운영 배포 검증 완료 | 2026-09-27 | 1 |
| D. 1단계 구현 | 1단계·공유 이미지 main 푸시/운영 확인. 집계 공통 캐시 보완 검증 완료 | 2026-09-21 | 1 |
...
사용자가 이미지의 1단계 네 항목을 순차 구현·검증·커밋·푸시 요청. 최신 원격 main 9167168을 확인하고 /private/tmp/kota-stage1-20260921, codex/stage1-20260921에서 작업. 다른 작업 폴더 미커밋 변경은 건드리지 않음.

1. 종료 표시·기본 목록 종료 제외는 기존 구현 확인. 종료 상세의 CTA를 실제 진행 중 목록으로 연결. 종료 검색정보에서 EventScheduled 제거. 다음 회차 달력 구독 API 추가: 새 일정이 없으면 빈 달력, 발표된 회차만 시작 하루 전 알림. URL 구독 필요·앱별 갱신/알림 차이 명시. 단위 18검사 통과.
2. 메뉴 한·영·일·태 표시 사전, 한국어 원문 병기, 명시된 돼지고기/매운맛 표시 구현. 미등록 고유 메뉴는 원문을 보존하고 언어가 지정된 Google 번역 링크 제공. 모든 메뉴를 자체 번역한 것은 아님. 주소 크게 보여주기 추가. 메뉴 단위검사 2개 및 TypeScript 통과. 다음: 확인 날짜/축제 수, 문의.

3. 확인 기록을 상세 상단에 표시: 운영 안내 확인·일정 확인·공공데이터 동기화 날짜를 분리하고 근거 없는 경우 미확인 표시. 기존 정정표의 확인일 기록을 구조화해 표시. 홈/기본 목록/사이트맵이 같은 activeCatalog(종료 제외·상시 포함·대표 ID 중복 제거)를 사용. 사이트맵 1시간 재생성. 날짜/집계 단위검사 3개와 TypeScript 통과.

4. 공식 홈페이지·SNS·기록된 이메일을 전화보다 먼저 표시. 전화 안내 문구와 실제 번호 링크 분리. 외국어 응대 미확인 명시. 문의 정보 없는 축제와 한국어판에도 1330 국내/해외 전화·공식 채팅 안내 제공. 한국관광공사 공식 페이지 2026-09-21 확인(https://english.visitkorea.or.kr/svc/contents/infoHtmlView.do?menuSn=454&vcontsId=140632). 24시간 보장 문구는 제거하고 언어별 운영 안내 링크 제공. 단위검사 및 TypeScript 통과. 전체 검증 예정.

5. 사용자 추가 요청(2026-09-21 20:40): 카카오 공유에 옛 당근 로고가 나타남. 운영 /og.png는 이미 새 로고·창살 이미지였고 로컬 파일과 SHA256 일치(128c3f04…). /og-korea-festa-20260921.png로 새 경로를 만들어 OG/Twitter/홈 구조화 정보/카카오 직접공유 fallback을 통일. 카카오 개발자 캐시 도구는 로그인 필요하여 사용자에게 요청함. 기존 메시지의 미리보기 변경까지 완료했다고 단정하지 말 것.
6. 검증: 웹137검사·수집8검사 통과, lint 오류0(기존 Poster img 경고1), TypeScript 통과. 기본 Turbopack은 로컬 포트 생성 EPERM으로 중단되어 공식 webpack 모드로 빌드: 2649/2649 성공. 한·영·일·태 390px 상세 화면 가로 넘침0, 달력 구독·종료 상태·연락처 없는 축제 1330 확인. 홈/목록/사이트맵 각각 447건 일치(빌드 시점). 달력 API 빈 일정200·실제 미래 회차200·잘못된 날짜400 확인. 화면 검증에서 무료/일부 체험 유료 문구 번역 추가.

7. 2026-09-22 10:42: d22064a Vercel 운영 배포 success 및 실제 새 OG 경로·일본어 요금·알림·문의 반영 확인. 운영 home488/list489/sitemap488 차이 발견: 페이지별 독립 재생성 캐시 시점이 달랐음. Next Data Cache에 원천 병합 결과를 공유하고 목록 화면은 요청 시 공통 캐시를 읽도록 보완(currentCatalog). 원천 조회는 1시간 캐시 유지, 한국 날짜별 키로 날짜 전환 분리. server-only 목록 조회를 list-source로 분리해 브라우저 코드에 서버 API가 섞이지 않도록 함. 보완 후137검사·TypeScript·lint·2649페이지 빌드 통과. 로컬 HTTP home/list/sitemap447 일치. 재발 검증: `node web/scripts/check-catalog.mjs https://ko-ta.co.kr`. 상세 검증 보고서 docs/reviews/stage1-validation-2026-09-21.md.
...
- 모바일 외국어 전환 불가 제보에 따라 수정. 제보 기기·브라우저와 정확한 증상은 아직 미확인.
- 기존 메뉴가 hover/focus-within에만 의존. 클릭 시 버튼에 초점을 주지 않는 터치 브라우저에서 열리지 않을 가능성이 있어 native details/summary로 교체. 자바스크립트 실행 전에도 여닫을 수 있다.
- 운영 영어 화면 320px에서 문서 폭 360px로 언어 버튼 잘림을 재현. 모바일 로고·메뉴 간격을 조정해 문서 폭 320px 확인.
- 언어 선택 링크 높이 44px 확보. 기존 경로·검색 조건 유지 로직은 유지.
- 수정 파일: web/components/Header.tsx, web/components/LanguageSwitcher.tsx.
- 검증: 자동 검사 137/137 통과, 프로덕션 빌드 통과. 브라우저 320px에서 4개 언어 이동과 메뉴 열기/닫기·Enter 조작 확인. 390px 목록에서 period=weekend&region=seoul 유지한 영어 전환 확인.
- 실제 iPhone Safari 기기 검증은 미실시. 제보자의 동일 기기 재확인 필요.
- 수정 커밋 `3170b98` main 푸시 및 Vercel 배포 성공. 2026-09-27 운영 ko-ta.co.kr에서 한국어→영어→일본어→태국어 전환, 320px 문서 폭·언어 버튼 표시 확인.
- 린트 오류 0건. 기존 Poster.tsx 이미지 경고 1건은 이번 변경과 무관.
