...
|---|---|---|---:|
| C. 전체 품질 개선 | 구현·검증 완료 단계, 운영 미반영. 포스터 실확보 후속 필요 | 2026-09-11 07:20 | 1 |
...
`codex/site-quality-20260911`에서 검색·달력·중복·상세·다국어·이미지 수집 경로 개선.
운영 DB 쓰기와 main 배포는 하지 않았다. 원본 사용자 AGENTS.md 미추적 파일 보존.
작업 폴더: `/Users/piglet/Documents/ChatGPT/한국관광공사 지역축제 웹앱 KOTA/kota2-improvements`.
결과·전체 후속 계획 → `docs/reviews/site-improvement-2026-09-11.md`.

- 공개 데이터 개선본: 진행·예정554개, 이미지 없음165개, 주말119개 중27개 없음.
- 중복6쌍을 한 카드로 표시했기 때문에 누락이6개 감소. 새 포스터6장 확보가 아님.
- 갤러리40회 조회 복사본 실험: 새 커버0건. 데이터는 운영에 미반영.
- web71 + pipeline7 + media8 =86개 검사 통과. TypeScript 통과.
- 기본 빌드 로컬 포트 권한 실패, webpack 배포 빌드 성공. 모바일390px/필터복원 검사.

## 다음 순서

1. 브랜치 검토 후 운영 반영. 기본 빌드의 배포환경 결과 확인.
2. 주말 누락27건의 공식 상세 공지·현재 회차 이미지 확보/사용 근거 검수.
3. 나머지 포스터·날짜/시간·예약/교통·다국어 후속은 위 작업표 순서.
