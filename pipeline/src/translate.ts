...
// 지금은 문장 앞 두 조각까지만 옮기고, 그 이상은 원문을 둔다 — 어설픈 기계 번역보다
// 한국어 원문이 낫다는 판단(외국인은 번역 앱으로 원문을 읽는다). 이 부분은 다음 단계에서
// LLM 번역으로 교체한다.
...
for (const f of items) {
  ...
  f.translations = LANGS.map((lang: Lang) => {
    ...
    return {
      ...
      summary: hl?.summary ?? sumTr?.[lang] ?? null,
...
console.log(`▶ 번역 ${n}건 × 3언어 · 손번역 적용 ${handCount}건 · 요약 번역 ${withSum}건(산문 ${items.filter((f) => (f.summary?.length ?? 0) > MAX_ENGINE_SUMMARY).length}건은 원문 유지)`)
