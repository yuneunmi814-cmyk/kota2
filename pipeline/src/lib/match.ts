...
// 같은 축제인데 이름이 아예 다른 것들 — 규칙으로는 못 잡는다.
//
// 자라섬재즈페스티벌은 표준데이터에 「재즈페스티벌 in 가평」으로, 우리 수기 등록에는
// 「제23회 자라섬재즈페스티벌」로 들어와 있었다. 기간(10/09~11)도 장소도 같은 한 축제인데
// 정규화하면 '재즈in가평'과 '자라섬재즈'가 되어 겹치는 글자가 없다. 포함 관계도 아니다.
// 마누스가 두 줄에 같은 포스터를 붙여 보내는 바람에 알았다(2026-08-19).
//
// 이런 것은 규칙을 더 깎아서 잡으려 하면 안 된다 — 느슨하게 만들수록 다른 축제끼리
// 붙는다. 아는 것만 손으로 적는 편이 안전하다. 왼쪽을 오른쪽으로 읽는다.
const SAME: Record<string, string> = {
  재즈in가평: '자라섬재즈',
}

export function normalizeName(s: string): string {
  const n = rawNormalize(s)
  return SAME[n] ?? n
}

function rawNormalize(s: string): string {
  return s
    .replace(/[\(（\[].*?[\)）\]]/g, '') // 괄호 안
