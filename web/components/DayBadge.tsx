...
// 사진 왼쪽 위 시간 배지. 장기 행사는 당일 운영 여부를 보장하지 않는다.
//
// 한 자리를 나눠 쓴다. 동시에 뜨는 일은 없다(하나의 축제는 한 상태다).
  ...
  const label =
    badge.kind === 'endsToday'
      ...
      : badge.kind === 'countdown'
        ...
        : t(lang, badge.kind === 'inPeriod' ? 'status.inPeriod' : 'status.ongoing')

  return (
    <span
      title={badge.kind === 'inPeriod' ? t(lang, 'detail.scheduleNote') : undefined}
